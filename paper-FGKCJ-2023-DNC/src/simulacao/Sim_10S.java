...
public class Sim_10S {
	
	private static final double sc_R = 1E+9;
	private static final double sc_T = 0.001;
	...
	public static void rede_tfa_sfa() throws Exception {
		...
		ServiceCurve sc = CurvePwAffine.getFactory().createRateLatency(sc_R,sc_T);
		...
		Server s2 = network.addServer(sc);
		Server s3 = network.addServer(sc);
		Server s4 = network.addServer(sc);
		Server s5 = network.addServer(sc);
		Server s6 = network.addServer(sc);
		Server s7 = network.addServer(sc);
		Server s8 = network.addServer(sc);
		Server s9 = network.addServer(sc);
		...
		network.addLink(s6, s7);
		network.addLink(s7, s8);
		network.addLink(s8, s9);
		...
		Flow flow1 = network.addFlow(ac1, s0, s9);
		Flow flow2 = network.addFlow(ac2, s0, s9);
...
//	      }   
		...
		Flow flow1 = network.addFlow(ac1, s0, s9); // De s0 até s1, ou seja, inicio e fim (ex: s0,s2)
		Flow flow2 = network.addFlow(ac2, s0, s9);
