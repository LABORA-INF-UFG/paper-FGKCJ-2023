...
public class Delay_sigma_5hops {
	/* 
	 * Comparar com Omnet++
	 * 18.10.2022
	 */	
	...
	private static final double ac_b_0 = 1024; //tamanho do pkt
...
//	private static final double phi_R5 = 0.075; // percentual do servidor alocado ao urllc
	...
	public static double rede_sfa(int N,int sigma) throws Exception {
		...
		ArrivalCurve ac = CurvePwAffine.getFactory().createTokenBucket(ac_r_0, sigma*ac_b_0);
...
public static double rede_tfa(int N,int sigma) throws Exception {
...
public static double rede_pmoo(int N,int sigma) throws Exception {
	...
	public static double fifo_sfa(int N,int sigma) {
...
//		config.setArrivalBoundMethod(AnalysisConfig.ArrivalBoundMethod.PBOO_CONCATENATION);
	...
	public static double fifo_tfa(int N,int sigma) {
		config = new AnalysisConfig();
		config.setMultiplexingDiscipline(AnalysisConfig.MuxDiscipline.GLOBAL_FIFO);
//		config.setArrivalBoundMethod(AnalysisConfig.ArrivalBoundMethod.PBOO_CONCATENATION);
        config.setUseGamma(AnalysisConfig.GammaFlag.GLOBALLY_OFF);
	...
	public static double blind_sfa(int N,int sigma) {
		config = new AnalysisConfig();
...
//		config.setArrivalBoundMethod(AnalysisConfig.ArrivalBoundMethod.PMOO);
	...
	public static double blind_tfa(int N,int sigma) {
		config = new AnalysisConfig();
		config.setMultiplexingDiscipline(AnalysisConfig.MuxDiscipline.GLOBAL_ARBITRARY);
//		config.setArrivalBoundMethod(AnalysisConfig.ArrivalBoundMethod.PMOO);
        config.setUseGamma(AnalysisConfig.GammaFlag.GLOBALLY_OFF);
	...
	public static double blind_pmoo(int N,int sigma) {
		config = new AnalysisConfig();
//		config.setMultiplexingDiscipline(AnalysisConfig.MuxDiscipline.GLOBAL_ARBITRARY);
		config.setArrivalBoundMethod(AnalysisConfig.ArrivalBoundMethod.PMOO);
        config.setUseGamma(AnalysisConfig.GammaFlag.GLOBALLY_OFF);
	...
	public static void main(String[] args) {
		...
		try {
			int n=10; // Numero de UEs
			int sigma=1;//qtde de pkts por rajada
