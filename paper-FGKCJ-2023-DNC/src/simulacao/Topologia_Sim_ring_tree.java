...
public class Topologia_Sim_ring_tree {
	
	private static final double sc_R_s = 2.5E+9;
	private static final double sc_T_s = 39.8E-6;
	private static final double sc_R_t = 5E+9;
	private static final double sc_T_t = 2.4E-6;
	private static final double sc_R_u = 10E+9;
	private static final double sc_T_u = 36.2E-6;
...
//		network.addLink(s0, s1);
		...
		Flow[] flow_of_interest_u=new Flow[0];
		Flow[] flow_of_interest_e=new Flow[0];
		if (N_e!=0) {
		flow_of_interest_e=flow_e_s4;
		}
		if (N_u!=0) {
		flow_of_interest_u=flow_u_s4;
		}
...
//		System.out.println("Fluxos URLLC");
		if (print_all==true) {
			System.out.println("Fluxos URLLC");
		for (int i=0;i<N_u;i++) {
			analysis_sfa.performAnalysis(flow_of_interest_u[i]);
			System.out.println("SFA para fluxo " + i + " -> " + analysis_sfa.getDelayBound().doubleValue()*1E+6 + "us");
	}
		for (int i=0;i<N_u;i++) {
			analysis_tfa.performAnalysis(flow_of_interest_u[i]);
		...
		for (int i=0;i<N_e;i++) {
			...
			System.out.println("Fluxo de interesse URLLC");
				analysis_sfa.performAnalysis(flow_of_interest_u[0]);
				System.out.println("SFA -> " + analysis_sfa.getDelayBound().doubleValue()*1E+6 + " us");
	
				analysis_tfa.performAnalysis(flow_of_interest_u[0]);
...
//		System.out.println("Fluxos URLLC para PMOO");
		if (print_all==true) {
			System.out.println("Fluxos URLLC para PMOO");
		for (int i=0;i<N_u;i++) {
			analysis_pmoo.performAnalysis(flow_of_interest_u[i]);
		...
		for (int i=0;i<N_e;i++) {
			...
			System.out.println("Fluxo de interesse URLLC para PMOO");
				analysis_pmoo.performAnalysis(flow_of_interest_u[0]);
